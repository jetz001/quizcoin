...
library LibDiamond {
    ...
    struct DiamondStorage {
        mapping(bytes4 => FacetAddressAndPosition) facetAddressAndSelectorPosition;
    ...
    function setSelectors(
        ...
        if (_action == IDiamondCut.FacetCutAction.Add) {
            // แก้ไขตรงนี้: เปลี่ยนเป็น enforceHasContractCode
            enforceHasContractCode(_facetAddress, "LibDiamondCut: Can't add an address that has no contract code");
            for (uint256 i = 0; i < _selectors.length; i++) {
                bytes4 selector = _selectors[i];
                address oldFacetAddress = ds.facetAddressAndSelectorPosition[selector].facetAddress; 
                require(oldFacetAddress == address(0), "LibDiamondCut: Can't add function that already exists");
                ds.facetAddressAndSelectorPosition[selector].facetAddress = _facetAddress;
                ds.facetAddressAndSelectorPosition[selector].position = uint96(ds.selectors.length);
            ...
            for (uint256 i = 0; i < _selectors.length; i++) {
                ...
                require(oldFacetAddress == _facetAddress, "LibDiamondCut: Can't replace function with different facet address");
                ds.facetAddressAndSelectorPosition[selector].facetAddress = _facetAddress;
            }
            ...
            for (uint256 i = 0; i < _selectors.length; i++) {
                ...
                // Clear the selector
                ds.facetAddressAndSelectorPosition[selector].facetAddress = address(0);
                ...
                uint96 selectorPosition = ds.facetAddressAndSelectorPosition[selector].position;
                bytes4 lastSelector = ds.selectors[ds.selectors.length - 1];
                ds.selectors[selectorPosition] = lastSelector;
                ds.facetAddressAndSelectorPosition[lastSelector].position = selectorPosition;
                ds.selectors.pop();
            }
        } else {
